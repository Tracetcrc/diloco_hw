...
from utils.checkpoint_utils import save_dc_diloco_checkpoint, load_dc_diloco_checkpoint, get_latest_checkpoint
from utils.shard_utils import get_layer_shards



# 批量 all-reduce 操作以提高通信效率，实现传算分离
def sync_model(model, shard_tracker, sync_shard_idx, world_size, logger,
    ...
    # 把 w_corrected 赋值给 model
    with torch.no_grad():
        for local_p, global_p in zip(param_refs, w_corrected):
...
def train(local_rank, model, dataset, optimizer, scheduler,
    ...
    if rank == 0:
        logger.info(f"Layer shards: {num_layers} layers → {num_shards} shards, sync points: {base_sync_points}")
    for idx, param_list in enumerate(shard_params):
        param_list = param_list.cuda(local_rank)
        shard_tracker[idx] = {
        ...
        shard_tracker[idx]["outer_optimizer"] = None if args.outer_lr == 1.0 else SGD(
            shard_tracker[idx]["params"], lr=args.outer_lr,
            momentum=0.9, nesterov=bool(args.use_nesterov)
        )
    if rank == 0:  logger.info(f"模型共 {sum(len(p) for p in shard_params)} 个参数组，分为 {num_shards} 个分片")
    ...
    if args.resume and args.checkpoint_dir:
        ...
        if resume_path:
            ...
            # 加载检查点状态
            training_state = load_dc_diloco_checkpoint(
    ...
    for epoch_idx in range(epoch, args.epochs):
        ...
        for batch in train_dataloader:
            ...
            if micro_step % args.gradient_accumulation_steps == 0:
                ...
                # 每隔 checkpoint_interval 步，在GPU0上保存检查点
                if args.checkpoint_dir and global_step % args.checkpoint_interval == 0 and local_rank == 0:
                    # 所有rank都保存检查点                    
                    logger.info(f"Rank {rank}: 保存检查点，当前步数: {global_step}")
                    save_dc_diloco_checkpoint(
                ...
                for shard_idx in range(num_shards):
                    ...
                    if relative_step == base_sync_points[shard_idx]:
                        shard_tracker[shard_idx]["old_sent_at_step"] = shard_tracker[shard_idx]["sent_at_step"]
                        ...
                        shard_tracker[shard_idx]["staged_params"] = [p.data.clone() for p in shard_tracker[shard_idx]["param_refs"]]
...
def main():
    ...
    if rank == 0 and args.use_wandb:
        ...
        if args.wandb_name is None:
            args.wandb_name = f"dc_{args.model_name}_{args.dataset_name}_sync{args.sync_interval}_ns{args.num_shards}_ds{args.delay_steps}_lmd{args.dc_lambda}"
        ...
        try:
            wandb.init(
                ...
                dir='/data/yzhu/distrain/runs/dc_diloco',
                config=wandb_config,
    ...
    try:
        train(local_rank, model, (train_dataloader, eval_dataloader), optimizer,
